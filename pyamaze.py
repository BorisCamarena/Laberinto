...
Tanto Tk como tkinter están disponibles en la mayoría de las plataformas Unix, así como en sistemas Windows (Tk en sí no es parte de Python,

es mantenido por ActiveState).

Ejecutar python -m tkinter desde la línea de comandos debería abrir una ventana que demuestre una interfaz Tk simple para saber si tkinter está instalado

correctamente en su sistema. También muestra qué versión de Tcl/Tk está instalada para que pueda leer la documentación de Tcl/Tk específica de esa versión.

IMPORTANTE para la funcion principal del programa al final del codigo.
...
class COLOR(Enum):
    ...
    # Esta clase está creada para usar los colores de la biblioteca grafica tkinter ( color del laberinto ).
    # El resto de colores son para Agentes.
    # El primer valor es el color del Agente y el segundo es el color de
    # su huella
...
# Colocar los agentes en el laberinto.
# Pueden tener dos formas (cuadrado o flecha)
# Creamos nuestra clase agente.
# Codigo muy importantre por que creamos el agente y le damos coordenadas.
# posicion cabeza, cuerpo ( semejante a la viborita ).

class agent:

# Constructor del agente dentro del laberinto parentmaze.

# Colores definidos en COLOR.
...
# Una función decoradora ("decorator function" en inglés) 
# es una función que agrega funcionalidad a otra función que recibe como argumento.
...
# variable protegida.
    ...
    # setter del vector posicion (x,y)
...
# Clase punto nuevo.
# rota el agente en sentido anti horario.
...
# Analogamente creamos la funcion que rota el agente en sentido horario.
...
# parentmaze--> El laberinto en el que se mostrará la etiqueta.
#        title--> El título del valor a mostrar
...
class maze:
    ...
    # Clase importante por que creamos el laberinto.
    
    def __init__(self,rows=10,cols=10):

        # constructor del laberinto por default 10x10.
    ...
    def _Open_East(self,x, y):
        ...
        # Manipulamos el laberinto.
        
        # Para quitar la pared este de la celda
    ...
    def CreateMaze(self,x=1,y=1,pattern=None,loopPercent=0,saveMaze=False,loadMaze=None,theme:COLOR=COLOR.dark):
        ...
        patrón --> Puede ser 'v' para vertical o 'h' para horizontal
                    Solo el aspecto visual del laberinto será más vertical/horizontal
                    Los pasajes estarán allí.
        loopPercent--> 0 significa que solo habrá un camino desde el principio hasta el objetivo (laberinto perfecto)
                        Un valor más alto significa que habrá varias rutas (bucles)
                        Cuanto mayor sea el valor (máx. 100), más serán los bucles
        saveMaze--> Para guardar el laberinto generado como archivo CSV para referencia futura.
        loadMaze--> Proporcione el archivo CSV para generar un laberinto deseado
        tema--> Oscuro o Claro ( opcional ).
...
# Quita la pared entre dos celdas.
...
# Funcion importante, determina si hay un ciclo en el laberinto.
                    
        def isCyclic(cell1,cell2):
# Para evitar demasiada ruta en blanco (clara).
            ans=False
            if cell1[0]==cell2[0]:
...
# Busqueda en anchura.
        ...
        def BFS(cell):
            '''
            Búsqueda en anchura
            Para generar el camino más corto. Se puede mejorar con analisis de algoritmos
            Esto se usará solo cuando haya varias rutas (loopPercent>0) o
            Maze se carga desde un archivo CSV donde se guarda el laberinto.
            Si se genera un laberinto perfecto y sin el archivo de carga, este método
            no se utilizará ya que la generación Maze calculará la ruta.
            '''
            frontier = deque()
        ...
        # cuando el laberinto no se genero aleatoriamente.
        ...
        if not loadMaze:
            ...
            # Mencionamos la funcion si es ciclico.

            # pero aqui tenemos la condicionante para ver si hay

            # multiples partes en loops.
...
# Eliminar bloques de las celdas de ruta más corta
                ...
                count=0
                i=0
                while count<count1: # Bloques a eliminar.
                    ...
                    if i==len(pathCells):
                        break
                # Remover bloques en partes de las celdas.
                
                if len(notPathCells)>0:
                    count=0
                    i=0
                    while count<count2: # Bloques a remover.
            ...
            # Carga el laberinto guardado en el archivo CSV.
...
# Creación de líneas de laberinto y ventana Tkinter previamente esplicado.
        ...
        # Cálculos para calcular el ancho de la celda del laberinto ( Ajustar el laberinto ).
        ...
        # Creacion de las lineas del labrinto.
...
# Redibujar una celda con el agente cuadrado.

    def _redrawCell(self,x,y,theme):
        
        '''
        Redibujar una celda.
        Con el agente cuadrado de tamaño completo, puede superponerse con líneas de laberinto
        Entonces, la celda se vuelve a dibujar para que las líneas de celda estén en la parte superior.
        '''
...
# Para controlar un agente a con teclas de flecha.
...
# Para controlar una agente a con las teclas W, A, S, D.
...
# La funcion siguiente es una derivacion.

# de python para trazar una ruta ( Traceroute ).
    
    def _tracePathSingle(self,a,p,kill,showMarked,delay):

        # Ayudar al método tracePath a rastrear una ruta por agente.
        
        def killAgent(a):

# Si el agente debe morir después de que alcance la meta o complete la ruta
        ...
        # Si la ruta se proporciona como Diccionario.
        ...
        # Si la ruta se proporciona como cadena ( str ).
...
# Si la ruta se proporciona como una lista.
        ...
        # Funcion rastrear ruta del agente.

    def tracePath(self,d,kill=False,delay=300,showMarked=False):
        
        # Rastrear la ruta por agente
...
# Se ejecuta el Tkinter Main Loop
